...
def run_program():
    ...
    for day in days:
        ...
        for file in demand_csv_files:
            ...
            month = file_name_without_ext.split("-")[1]
            # veh_file = os.path.join(veh_folder, f"{city_id}_unique_vehid", day, file_name)
            veh_file = os.path.join(veh_folder, f"{city_id}_unique_vehid", "common_folder", "new_folder", f"all_{month}.csv")
            veh_file_df = pd.read_csv(veh_file)
            veh_list = []
            active_fleetsize_dict = {}
            for index, row in veh_file_df.tail(1).iterrows():
            ...
            day_op_cancellation_rate_list.append(100 - df["MoD_0"][11])
            ...
            day_avg_travel_time_list.append(df["MoD_0"][15])
        ...
        df_day = pd.DataFrame({"date": date_list, "op_cancellation_rate": day_op_cancellation_rate_list,
                               ...
                               "avg_tt" : day_avg_travel_time_list})
...
N_BATCH = 15  # Number of iterations in optimization loop   # used 30 before
BATCH_SIZE = 1  # Number of candidate points to be generated   # used 2 before
n_samples = 10  # Initial sobol samples for training             # used 15 before
...
param_no_show_wait_time = RangeParameter(name="no_show_wait_time", lower=30.0, upper=90.0,
                                         parameter_type=ParameterType.FLOAT)
param_max_rel_error_black_box_PUDO_dur = RangeParameter(name="max_rel_error_black_box_PUDO_dur", lower=2.0, upper=2.5,
